...
import { SELECTION_CHANGE, SELECTION_ITEMS_CHANGE } from "@fluentui/utilities";

export class FluentSelection<TItem = IObjectWithKey>
  implements ISelection<TItem>
{
  ...
  constructor(
    ...
    this._getKey = getKey || ((i: any) => i.id ?? i.key);
    ...
    for (let i = 0; i < items.length; i++) {
      ...
      if (item) {
        ...
        if (key) {
          if (
            !haveItemsChanged &&
            (!(key in this._keyToIndexMap) || this._keyToIndexMap[key] !== i)
          ) {
  ...
  public setAllSelected(isAllSelected: boolean): void {
    ...
    const selectableCount = this._items
  ...
  public setRangeSelected(
    ...
    for (; startIndex <= endIndex; startIndex++) {
      this.setIndexSelected(
        startIndex,
        isSelected,
        shouldAnchor ? startIndex === newAnchorIndex : false
      );
    }
  ...
  private _setAllSelected(
    ...
    preserveModalState: boolean = false
  ): void {
    if (isAllSelected && this.mode !== SelectionMode.multiple) {
      return;
    }
